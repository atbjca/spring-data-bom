...
- [x] 1.1 Confirm this change is apply-ready and assigned exclusively to `spring-data-bom-3.5` in `spring-data-bom-3.5`.
- [x] 1.2 Acquire the component owner lease and record owner/session/timestamps in the release manifest.
- [x] 1.3 Record branch, `origin` fetch/push URLs, HEAD SHA, active OpenSpec changes, toolchain, current version, and target `2025.0.13-nes.patch.1` without exposing credentials.
...
- [x] 1.5 Confirm all declared upstream components are Nexus-verified RELEASEs and record their evidence: `spring-data-commons-3.5`, `spring-data-keyvalue-3.5`, `spring-data-redis-3.5`, `spring-data-elasticsearch-3.5`.
- [x] 1.6 Confirm the approved explicit exclusions and how each will be verified: `root aggregator POM and bom-client are not deployable release artifacts`.

## 2. RELEASE preparation and component documentation

- [x] 2.1 Set the component version to `2025.0.13-nes.patch.1`.
...
- [x] 2.3 Discover the complete local publication set and reconcile it with the representative GAVs: `cn.bjca.footstone.bpring.data:bjca-footstone-bpring-data-bom:2025.0.13-nes.patch.1`.
...
- [x] 2.5 Update `README.adoc` with version, GAVs, required RELEASE dependencies, validation commands, Nexus location, exclusions, and notable constraints.
- [x] 2.6 Review the full working diff and confirm it contains no unrelated source changes or credentials.

## 3. Local verification

- [x] 3.1 Audit and reuse BOM packaging evidence under the release-only metadata exception; do not rerun `make build`.
- [x] 3.2 BOM consumer smoke via local import replaces `make verify-client` for RELEASE metadata verification.
- [x] 3.3 Generate local publications with `./mvnw -pl bom -am -DskipTests -Dmaven.test.skip=true install` (no clean).
...
- [x] 4.3 Create the dedicated release commit for `spring-data-bom-3.5` `2025.0.13-nes.patch.1`.
...
- [x] 7.1 Confirm `v2025.0.13-nes.patch.1` does not already exist locally or remotely and the target release commit SHA is unchanged.
- [x] 7.2 Create annotated tag `v2025.0.13-nes.patch.1` on the exact release commit only after `nexus-verified`.
