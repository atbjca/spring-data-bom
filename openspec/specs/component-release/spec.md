...
## Purpose
TBD - created by archiving change release-2021-2-18-nes-patch-1. Update Purpose after archive.
## Requirements
### Requirement: Exclusive and safe component preparation

The release process MUST assign one active owner to `spring-data-bom-2.7` and MUST inspect branch, origin, tracked and untracked changes, and active OpenSpec changes before modifying `spring-data-bom-2.7`.
...
Every generated RELEASE POM or BOM for `2021.2.18-nes.patch.1` MUST use approved RELEASE versions for internal `cn.bjca.footstone` dependencies, parents, imported BOMs, and plugins.
...
- **WHEN** `make build`, `make client`, `make install`, the metadata scan, and consumer validation succeed
...
The component SHALL have one dedicated release commit containing `2021.2.18-nes.patch.1`, internal RELEASE dependency updates, `README.adoc`, approved exclusions, and no unrelated source changes.
...
- **WHEN** no POM, binary, checksum, metadata, or partial module asset exists for `2021.2.18-nes.patch.1`
...
After Nexus verification, the coordinator MUST create annotated tag `v2021.2.18-nes.patch.1` on the exact release commit used to build and deploy `2021.2.18-nes.patch.1`.
